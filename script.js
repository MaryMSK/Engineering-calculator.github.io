...
//Корень квадратный, в квадрат, в -1 степень, в любую степень
...
function log(name) {
    if (name == 'lg') {
        input.textContent = parseFloat(Math.log10(eval(input.textContent)).toFixed(8).toString());
    }
    if (name == 'ln') {
        input.textContent = parseFloat(Math.log(eval(input.textContent)).toFixed(8).toString());
    }
}
...
//Синусы и косинусы; Тангенсы и Катангенсы
function f(name) {
    if (name == 'sin') {
        if (document.querySelector('.type').textContent == "deg") {
            //В градусах
